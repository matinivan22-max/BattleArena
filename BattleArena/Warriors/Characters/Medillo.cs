...
{
    public class Medillo : Warrior, IHealCaster
    {
        public int HealingAmount { get; set; }
        public Medillo(int health, int attackPower, TeamType teamType, int healingAmount)
            : base("Medillo", health, attackPower, WarriorType.Magery, teamType)
        {
        ...
        public override void Attack(Warrior target)
        {
            ...
            Thread.Sleep(1000);
            if (target.IsAlive)
                Console.WriteLine($"->{target.Name}: Mama!! {Name}");
        }
